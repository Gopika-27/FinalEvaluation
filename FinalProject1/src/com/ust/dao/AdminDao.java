...
public class AdminDao {
	...
	public Login verifylogin(String username, String password) {
		String sql = "select userId,username,password from loginTable where username='"
	...
	public List<VendorContact> getAllVendor() {
		return template
				.query("select vendorid,vendorname,vendoraddress,vendorlocation,vendorservice,vendorpincode from vendorTable where isActive=0",
	...
	public List<VendorContact> getVendorByName(String vendorname) {
		return template
				.query("select vendorid,vendorname,vendoraddress,vendorlocation,vendorservice,vendorpincode from vendorTable where isActive=0 and vendorname='"
	...
	public int addVendor(VendorContact vendorContact) {
		...
		String sql1 = "insert into vendorTable(vendorname,vendoraddress,vendorlocation,vendorservice,vendorpincode,isActive) values "
				...
				+ "',"
				+ vendorContact.getVendorPincode() + "," + 0 + ")";
	...
	public int updateVendor(int vendorid, VendorContact vendorCont) {
		...
		String sql = "update vendorTable set vendorname='"
				+ vendorCont.getVendorName() + "' ,vendoraddress='"
				+ vendorCont.getVendorAddress() + "' ,vendorlocation='"
				+ vendorCont.getVendorLocation() + "',vendorservice='"
				+ vendorCont.getVendorService() + "',vendorpincode='"
				+ vendorCont.getVendorPincode() + "',isActive="
				+ vendorCont.getIsActive() + " " + "where vendorid ="
				+ vendorid;
		template.update(sql);

		// Integer maxId = getSequence();

		String sql1 = "update contactTable set vendorid=" + vendorid
				+ ",contactname='" + vendorCont.getContactDepartment()
				+ "',contactDepartment='" + vendorCont.getContactName()
				+ "',contactEmail='" + vendorCont.getContactEmail()
				+ "',contactPhone='" + vendorCont.getContactPhone()
				+ "'where contactid = " + vendorCont.getContactId();
	...
	public int disableVendor(int vendorId) {
		...
		String sql = "update vendorTable set isActive='1' where vendorId="
				+ vendorId + "";

		return template.update(sql);
	}

	/*-----------------------FIND CONTACT BY VENDORID------------------*/

	public VendorContact getVendorById(int vendorId) {

		String sql = "select vc.vendorid,vc.vendorname,vc.vendoraddress,vc.vendorservice,vc.vendorpincode,cd.contactname,cd.contactDepartment,cd.contactEmail,cd.contactPhone,cd.contactid from vendorTable vc join contactTable cd on vc.vendorid=cd.vendorid where vc.isActive=0 and vc.vendorid='"
				+ vendorId + "'";
		return template.queryForObject(sql, new Object[] {},
